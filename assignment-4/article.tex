...
\begin{align*}
  ...
  &= \frac{1}{\sqrt{2\pi}} \exp \left[\log(x(1-x)))^{-\frac{1}{2} \cdot 2} \right] \cdot \frac{1-2x}{x(1-x)} \\
  &= \frac{1}{\sqrt{2\pi}} \exp \left[\log\left(\frac{1}{x(1-x)}\right) \right] \cdot \frac{1-2x}{x(1-x)} \\
  &= \frac{1}{\sqrt{2\pi}} \frac{1}{x(1-x)} \cdot \frac{1-2x}{x(1-x)} \\
  &= \frac{1}{\sqrt{2\pi}} \frac{1-2x}{x^2(1-x)^2}
\end{align*}

%% \subsubsection*{Problem 16.2 solution - attempt 2}
...
%% We can derive the inverse function $f^{-1}(x)$:
%% \begin{equation*}
%%   z = f^{-1}(x) = -\log \left(\frac{1}{x} - 1\right)
%% \end{equation*}

%% The formula for the change of variables in probability densities is:

%% \begin{equation*}
%%   Pr(x) = Pr(f^{-1}(x)) \left| \frac{df^{-1}(x)}{dx} \right|
%% \end{equation*}

%% By differentiating $f^{-1}(x)$ with respect to $x$, we get $\frac{df^{-1}(x)}{dx} = \frac{1}{x(1-x)}$.

%% Substituting $f^{-1}(x)$ and $\frac{df^{-1}(x)}{dx}$ into the formula, we get:

%% \begin{equation*}
%%   Pr(x) = \frac{1}{\sqrt{2\pi}} \exp \left[-\frac{(-\ln(\frac{1}{x} - 1))^2}{2}\right] \cdot \frac{1}{x(1-x)}
%% \end{equation*}

%% This is the probability density function of the transformed distribution.
